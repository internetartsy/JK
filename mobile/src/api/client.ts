...
const getBaseUrl = () => {
    // Check if running in Expo Go (physical device vs simulator) might need explicit IP
    // For local dev, we start with standard mappings:
    if (Platform.OS === 'android') {
        return 'http://10.0.2.2:8090/api/v1'; // Routed via Rust Security Gateway
    }
    // iOS and Web
